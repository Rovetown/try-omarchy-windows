# Historical session notes through September 13, 2026

Archived for evidence and recovery context. Start with
[the current handoff](SESSION-RESUME.md) and [v1 readiness](V1-READINESS.md).
Release state, branch names and continuation priorities below are historical;
they do not define the current v1 scope. Retain the recorded Windows cleanup
restrictions when working with those same recovery artifacts.
